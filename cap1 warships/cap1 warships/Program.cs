...
class Program
{
    ...
    private static void PlaceShip(ref char[,] Board, ShipType[] Ships, int Row, int Column, char Orientation, int I)
    {
        if (Orientation == 'v')
        {
            ...
            {
                Board[Row + Scan, Column] = Ships[I].Name[0];
            }
        }
        else if (Orientation == 'h')
        {
            for (int Scan = 0; Scan < Ships[I].Size; Scan++)
            {
                Board[Row, Column + Scan] = Ships[I].Name[0];
            }
        }
    }
    ...
    {
        string chars = ("WELCOME TO WARSHIPS " + name +"\nMAIN MENU\n\n1. Start new game\n\n2. Load training game\n\n3. patches\n\n9. Quit\n\n");
    ...
    static void Main(string[] args) 
    {
        ...
        AskName(ref name);
        int MenuOption = 0;
        while (MenuOption != 9 || (MenuOption > 3  && MenuOption < 1))
        {
            try
            {
