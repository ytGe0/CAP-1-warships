...
class Program
{
    ...
    static public int FlareSize = 3; //flare size
    ...
    private static void MakePlayerMove(ref int Tries, ref char[,] Board, ref ShipType[] Ships)
    {
        ...
        if (Tries % 10 == 0 && Tries > 0)
        {
            ...
            Console.Write("The smoke from the flare has affected your ability to know when ships have sunk!");
            for (int i = -1; i < FlareSize - 1; i++)
            {
                for (int j = -1; j < FlareSize - 1; j++)
                {
                    try
                    {
                        if (Board[Column + i, Row + j] == 'A' || Board[Column + i, Row + j] == 'B' || Board[Column + i, Row + j] == 'S' || Board[Column + i, Row + j] == 'D' || Board[Column + i, Row + j] == 'P')
                        {
    ...
    static void Main(string[] args)
    {
        ...
        AskName(ref name);
        int MenuOption = 0;
        while (MenuOption != 9 || (MenuOption > 4 || MenuOption < 1))
        {
            try
            {
                if (MenuOption == '1')
